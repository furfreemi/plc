...
(define addtotop
  (lambda (var val state)
    (cons (cons (cons var (toplayer_vars state)) (cons (cons val (toplayer_vals state)) '())) (remaining state))))
    ...
    (call/cc
     (lambda (return)
      (cond
        ...
        ((eq? (tree_funcname tree) 'main) (evalParseTree (remaining_exp (main_body tree)) (M_state (first_exp (main_body tree)) (add_scope state) invalid_break invalid_continue return invalid_throw)))
        (else (evalParseTree (remaining_exp tree) (M_state (first_exp tree) state invalid_break invalid_continue return invalid_throw)))
    )))))

(define tree_funcname cadar)
(define main_body cadddar)
...
(define lookup
  (lambda (exp state)
    (cond
      ...
      ((not (list? (top_state state))) (top_state state))
      ((null? (toplayer_vars state)) (lookup exp (remaining state)))
...
(define global_state
  (lambda (state)
    (if (null? (remaining state))
        (top_state state)
        (global_state (remaining state)))))
...
(lambda (var state)
    (cond
      ...
      ((eq? '() (toplayer_vars state)) (cons (topstate state) (attach_variable var (poplayer state))))
      (else (addstate (toplayer_vars state) (toplayer_vals state) (attach_variable var (build_modified_state (toplayer_vars (remaining state)) (toplayer_vals (remaining state)) (lower_layers (remaining state))))
                      )))))
  ...
  (lambda (exp state break continue return throw)
    (letrec ((M_stateloop (lambda (exp state break continue return throw)
      (cond
        ...
        ((eq? (operand exp) '=) (if (and (declared? (getvar exp) state) (not (eq? (op2 exp state throw) 'UNDECLARED)))
                                    (change_value (getvar exp) (op2 exp state throw) state)
...
(define M_state_funcall
  (lambda (exp state break continue return throw)
    ...
    (cadr (state_run_helper (cadr (lookup (cadr exp) state)) (cons (bind_func_vars (car (lookup (cadr exp) state)) (cddr exp) (cons '() (cons '() '())) (make_static_state state) throw) (cons (get_local_state (cadr exp) state) (cons (global_state state) '())))  break continue (lambda (v) (cadr v)) throw))))

; adds function to variable list: (funcname)->((params) (body) (localscope))
(define M_state_function
  (lambda (exp state break continue return throw)
    (addtotop (cadr exp) (cons (caddr exp) (cons (cadddr exp) (cons (define_local_scope state) '()))) state)))

; returns any non-global variables that will be in-scope at function runtime:
;if global- just returns empty state to avoid crashing
(define define_local_scope
  (lambda (state)
    (if (null? (cdr state)) '(() ())
        (car state))))
...
(define M_value
  (lambda (exp state throw)
    (cond
      ...
      ((eq? (operand exp) 'funcall) (state_run_helper (cadr (lookup (cadr exp) state)) (cons (bind_func_vars (car (lookup (cadr exp) state)) (cddr exp) (cons '() (cons '() '())) (make_static_state state) throw) (cons (get_local_state (cadr exp) state) (cons (global_state state) '())))  invalid_break invalid_continue (lambda (v) (return v)) throw))
...
; returns non-global vars in-scope for function call
(define get_local_state
  (lambda (func_name state)
    (function_env (lookup func_name state))))

(define function_env caddr)
...
(define bind_func_vars
  (lambda (vars vals new_scope state throw)
    (cond
      ...
      ((null? vars) (error 'error "Too many input values"))
      ((null? vals) (error 'error "Not enough input values"))
      (else (bind_func_vars (remaining_binds vars) (remaining_binds vals) (cons (cons (current_bind vars) (current_bind new_scope)) (cons (cons (get_value (M_value (current_bind vals) state throw)) (cadr new_scope)) '())) state throw)))))

(define remaining_binds cdr)
(define current_bind car)
