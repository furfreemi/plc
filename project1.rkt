    ...
    (call/cc
     (lambda (return)
      (cond
        ...
        (else (evalParseTree (remaining_exp tree) (M_state (first_exp tree) state 'error 'error return)))
    )))))
...
(define begin_helper
  (lambda (exp state break continue return)
    (cond
      ...
      (else (begin_helper (remaining_exp exp) (M_state (first_exp exp) state break continue return) break continue return)))))


;Mstate: takes in expression, passes to Mbool or Mval as necessary to evaluate
(define M_state
  (lambda (exp state break continue return)
    (letrec ((M_stateloop (lambda (exp state break continue return)
      (cond
        ((eq? (operand exp) 'var) (add (remaining_exp exp) state))
        ((eq? (operand exp) 'begin) (begin_helper (remaining_exp exp) (cons '(() ()) state) break continue return))
        ...
        ((eq? (operand exp) 'if) (if (M_boolean (condition exp) state)
                                   (M_stateloop (stmt1 exp) state break continue return)
                                   (if (eq? (length exp) 4)
                                       (M_stateloop (stmt2 exp) state break continue return)
                                        state))
                               )
        ((eq? (operand exp) 'while) (call/cc
                                   ...
                                   (if (M_boolean (condition exp2) state2)
                                      (loop exp2
                                       (call/cc
                                        (lambda (continue)
                                       (M_stateloop (stmt1 exp2) state2 break continue return))))
                                      state2
                                      )
                                  ))) (loop exp state)))))
      ...
      ((eq? (operand exp) 'return) (return (cons 'FINISH (cons (M_value (remaining_exp exp) state) '())))))))) (M_stateloop exp state break continue return))))
